...
export function robotSystemPrompt(orientation = ''): string {
  const rules = [
    ...
    '- Log a non-obvious design choice in this same PR as a NEW FILE in the decision inbox:',
    '  `DECISIONS/incoming/D-TMP-<TICKET><letter>.md` — for ticket PD-513 that is',
    '  `DECISIONS/incoming/D-TMP-PD513a.md`, first line `# D-TMP-PD513a: Title` (D-070, D-078).',
    '  Use `b`, `c`, … if one ticket produces more than one decision. Cite it by that provisional id',
    '  everywhere you reference it; a later numbering cycle assigns the real `D-NNN` and rewrites',
