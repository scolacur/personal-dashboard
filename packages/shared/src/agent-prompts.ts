...
export function robotSystemPrompt(orientation = ''): string {
  const rules = [
    ...
    '- Log a non-obvious design choice in this same PR as a NEW FILE, `DECISIONS/D-NNN-slug.md`,',
    '  whose first line is `# D-NNN: Title` (D-070). Take the next free number — the highest in the',
    '  index plus one. Then run `npm run decisions:index` to regenerate DECISIONS.md.',
...
export const ORIENTATION_SOURCES = [
  ...
  },
  {
    ...
    note: 'The generated index only (~80 lines of id + title + link). Decision bodies stay on-demand via Read.',
  },
  {
