...
### Step 2 — Curate memory from merged Sortie PRs

Sortie agents never write to MEMORY/ (concurrent merges would conflict on the daily
file). Instead, each agent's durable record lives in its PR (title, `Closes #N`,
`## Assumptions` block, and any `DECISIONS.md` entry). This step folds the non-obvious
parts of recently-merged Sortie work into a single curated memory entry.

1. Determine the boundary: the date of the most recent `MEMORY/YYYY-MM-DD.md` file.
2. List Sortie PRs merged since then:
   ```sh
   gh pr list --repo scolacur/personal-dashboard --state merged \
     ...
     --jq '.[] | select(.headRefName | startswith("sortie/"))'
   ```
...
4. Fold anything **non-obvious** (a surprising design choice, an assumption worth
   ...
   `## Sortie merges` subsection — ONE entry, written by you, not per-PR. Skip routine
