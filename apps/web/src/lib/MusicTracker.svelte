<script lang="ts">
  ...
  import { fetchTracks, createTrack } from './music-tracker/api';
  ...
  // Page variant shows both faces at once; widget variant shows one, driven by `view`.
  const showForm = $derived(variant === 'page' || view === 'generator');
  const showList = $derived(variant === 'page' || view === 'manage');
...
<div class="mt" data-variant={variant}>
  {#if variant === 'page'}
  ...
  {:else}
    {#if showForm}
      <form class="add-form" onsubmit={handleSubmit}>
        <div class="field-row">
          <div class="field">
            <label class="field-label" for="mt-artist">Artist</label>
            <input id="mt-artist" class="text-input" type="text" bind:value={artist} placeholder="Artist" />
          </div>
          <div class="field">
            <label class="field-label" for="mt-title">Title</label>
            <input id="mt-title" class="text-input" type="text" bind:value={title} placeholder="Title" />
          </div>
        </div>

        <div class="field">
          <label class="field-label" for="mt-remixer">Remixer / mix <span class="optional">(optional)</span></label>
          <input id="mt-remixer" class="text-input" type="text" bind:value={remixer} placeholder="e.g. Extended Mix" />
        </div>

        <div class="field">
          <label class="field-label" for="mt-notes">Notes <span class="optional">(optional)</span></label>
          <textarea id="mt-notes" class="text-input" rows="2" bind:value={notes} placeholder="Anything to remember"></textarea>
        </div>
    ...
    {#if showList}
      <section class="tracks">
        <h2 class="tracks-heading">
          Tracked <span class="count">({tracks.length})</span>
        </h2>

        {#if tracks.length === 0}
          <p class="status-msg">No tracks yet. Add one{variant === 'page' ? ' above' : ''}.</p>
        {:else}
          <ul class="track-list">
...
<style lang="scss" src="./MusicTracker.scss"></style>
