...
export function approveRefine(
  ...
  if (p.mode === 'refine_in_place') {
    ...
    // is recomputed from the body on write (never enforced here). D-080 retired the `prioritized`
  ...
  const run = db.transaction(() => {
    for (const c of children) {
      ...
      // D-080: the `prioritized` parking lane is retired, so a proposed-`queue` child parks in
      // `backlog`. Under D-080 a Ticket is not queued by hand at all — its Epic is — so PD-510
