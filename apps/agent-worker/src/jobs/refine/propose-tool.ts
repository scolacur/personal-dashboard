...
// D-080 retired `prioritized`. Kept in the *input* enum on purpose: an un-redeployed worker or a
  ...
  const proposeCommit = tool(
    ...
    async (args) => {
      const proposal: RefineProposal = {
        ...
        // D-080: fold a retired/legacy lane (`prioritized`, and the pre-D-058 queue lanes) to a
        ...
        ...(args.children !== undefined
          ? {
              children: args.children.map((c) => ({
                ...
                // Same D-080 lane fold as the parent status above, applied per child.
