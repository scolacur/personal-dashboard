...
describe('inMergeOrder', () => {
  ...
  it('orders by the commit that ADDED each file, not by filename', () => {
    ...
    const h = harness({ addTimes: { 'D-TMP-PD600a': 100, 'D-080': 200 } });
    return inMergeOrder([prov('D-080'), prov('D-TMP-PD600a')], h.deps, '/x').then((out) => {
      expect(out.map((d) => d.id)).toEqual(['D-TMP-PD600a', 'D-080']);
    });
  });

  it('sorts an undatable file last — it is almost always one added just now', async () => {
    const h = harness({ addTimes: { 'D-080': 200 } });
    const out = await inMergeOrder([prov('D-TMP-PD999a'), prov('D-080')], h.deps, '/x');
    expect(out.map((d) => d.id)).toEqual(['D-080', 'D-TMP-PD999a']);
  });
...
describe('runNumberingCycle', () => {
  ...
  it('numbers, moves, rewrites, opens a PR and admin-merges on green', async () => {
    const root = makeRepo([{ id: 'D-080', title: 'Epic dispatch' }]);
  ...
  it('refuses to rewrite when a run is somehow live inside the hold', async () => {
    ...
    const root = makeRepo([{ id: 'D-080', title: 'x' }]);
    ...
    expect(existsSync(path.join(root, 'DECISIONS/incoming/D-080.md'))).toBe(true);
